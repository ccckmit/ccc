...
var ftokens = file.split("/");
var fileName = ftokens[ftokens.length-1];
...
var segMap  = { LCL:1, ARG:2, THIS:3, THAT:4, TEMP:5 };
...
function cmdPush(type, arg) {
  if (type === "constant")
		...
		var typeSym = memType[type].label;
		memToStack(typeSym, arg);
	} else
		error();
	incSp();
}
...
function stackToReg(seg, i) {
	var segi = regNum(seg, i);
//	comment(" ==> regNum("+seg+","+i+")="+regNum(seg, i));
	stackToDest('D');
	compToReg(segi, 'D');
}
